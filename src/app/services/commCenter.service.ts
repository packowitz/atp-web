...
export class CommCenterService {
  ...
  deleteAnnouncement(announcement: Announcement): Observable<Boolean> {
    let headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.localStorage.getToken());
    return this.http.delete(environment.server + "/web/app/cc/announcement/" + announcement.id, {headers: headers}).map(() => true);
  }
}
