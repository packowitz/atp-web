...
export class SurveyService {
  ...
  createMultiPictureSurvey(survey: Survey, surveyType: SurveyType, pictures: string[], eachCountrySeparate: boolean): Observable<any> {
    let multiPictureSurvey = {
      survey: survey,
      type: surveyType.key,
      pictures: pictures,
      eachCountrySeparate: eachCountrySeparate
    };
    let headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.localStorage.getToken());
    headers.append('Content-Type', 'application/json');
    return this.http.post(environment.server + "/web/app/survey/multipicture", JSON.stringify(multiPictureSurvey), {headers: headers}).map(res => res.json());
  }
}
